...
void sql::init(){
	...
	if (_err != SQLITE_OK) {
		...
		std::cout << "\e[32mdata.db\e[39m loaded\n";
	}
}
...
int sql::l_create_table(lua_State* state){
	...
	if(args == 2){
		...
		if(len % 2 == 0){
			...
			_err = sqlite3_exec(sql::DB,sqlcom.c_str(),NULL,0,&sql::message_error);
			if (_err != SQLITE_OK) {
				lshow_error(state, sql::message_error);
				sqlite3_free(sql::message_error);
			}
		}
...
int sql::l_get(lua_State* state){
	...
	if(args == 2){
		...
		_err = sqlite3_prepare_v2(sql::DB, sqlcom.c_str(), -1, &stmt, NULL );
		if (_err != SQLITE_OK) {
			lshow_error(state, sqlcom + "\n" + sqlite3_errstr(sqlite3_errcode(sql::DB)));
...
int sql::l_update(lua_State* state){
	...
	if(args == 3){
		...
		sqlcom = sqlcom.substr(0, sqlcom.size()-5);
		sqlcom += ";";
		int _err = sqlite3_exec(sql::DB,sqlcom.c_str(),NULL,0,&sql::message_error);
		if (_err != SQLITE_OK) {
			lshow_error(state, sql::message_error);
			sqlite3_free(sql::message_error);
		}
